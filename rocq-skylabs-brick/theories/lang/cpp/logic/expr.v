...
Module Type Expr.
  ...
  Section with_resolve.
    ...
    Axiom wp_lval_assign : forall ty l r Q,
        (letI* '(la, rv), free :=
           eval2 (evaluation_order.order_of tu OOEqual) (wp_lval l) (wp_operand r) in
    ...
    *)
    ...
    Axiom wp_lval_call : forall f (es : list Expr) Q (ty := type_of (Ecall f es)),
        wp_call (evaluation_order.order_of tu OOCall) (type_of f) f es (fun res free =>
    ...
    Axiom wp_init_call : forall f es Q (addr : ptr) ty,
        (letI* res, free := wp_call (evaluation_order.order_of tu OOCall) (type_of f) f es in
    ...
    Axiom wp_lval_member_call : forall arrow ct fty f obj es vc cv nm Q,
        ...
        (let ty := type_of $ Emember_call arrow (inl (f, ct, fty)) obj es in
         let* res, free :=
           wp_mcall arrow (dispatch ct fty f $ tqualified cv (Tnamed nm)) (evaluation_order.order_of tu OOCall) obj fty es in
    ...
    Definition wp_operator_call (oo : OverloadableOperator) (oi : operator_impl) (es : list Expr) Q :=
      ...
      | operator_impl.Func f fty =>
          ...
          match args_for <$> as_function fty with
          | Some targs =>
            letI* fps, vs, ifree, free := wp_args (evaluation_order.order_of tu oo) [] targs es in
       ...
       | operator_impl.MFunc fn ct fty =>
           ...
           | Static =>
               ...
               | eobj :: es, Some targs =>
                   ...
                   in
                   letI* _fps, vs, ifree, free := wp_args (evaluation_order.order_of tu oo) [eval_obj] targs es in
           ...
           | Virtual =>
               match es with
               | eobj :: es =>
                   wp_mcall false (dispatch ct fty fn (type_of eobj)) (evaluation_order.order_of tu oo) eobj fty es Q
               | _ => False
               end
           end
      end%I.
