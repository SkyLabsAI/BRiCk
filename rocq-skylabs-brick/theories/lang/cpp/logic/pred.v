...
Module Type CPP_LOGIC
  ...
  Section with_cpp_logic.
     ...
     ** Deducing pointer equalities
     [same_address_eq_null] and [same_address_o_sub_eq] recover pointer
     identity in their respective null and same-array cases. General address
     equality (produced by C++ pointer comparison) does not establish object
     identity across lifetime changes.
     *)
    ...
    (** [type_ptr ty p] can survive the object's destruction, and [live_ptr p]
        only states that its allocation is alive. Even together, they do not
        establish that the pointed-to object is alive. In particular, they must
        not be used to identify storage pointers with placement-new objects
        merely because their addresses and types agree. *)
