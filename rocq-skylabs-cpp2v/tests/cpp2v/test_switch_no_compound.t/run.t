...
body in it, so the form below is what those macros expand to.
  ...
  $ . ../../setup-cpp2v.sh
  $ # Compare semantic AST text only: the two source-location sections differ.
  $ export CRAM_CPP2VFLAGS=--locations-inline=false
  $ check_cpp2v test.cpp
  cpp2v -v -check-types -o test_17_cpp.v test.cpp --locations-inline=false -- -std=c++17 2>&1 | sed 's/^ *[0-9]* | //'
...
of the same [Sseq], which is where [wp_switch] looks for them.
  ...
  cpp2v -v -check-types -o test_braced_17_cpp.v test_braced.cpp --locations-inline=false -- -std=c++17 2>&1 | sed 's/^ *[0-9]* | //'
