...
Require Import skylabs.lang.cpp.mparser (Nlocal).
...
Definition lookup_body (n : name) : option Stmt :=
...
Example paren_call_body :
  ...
  Some (Sseq [
    Sreturn (Some (Eunresolved_call (Nlocal (Nid "f")) []))
  ]).
Proof. vm_compute. reflexivity. Qed.
...
Example unary_plus_body :
  ...
  Some (Sseq [
    Sreturn (Some (Eunresolved_unop (Runop Uplus) (Evar "p" (Tparam "T"))))
  ]).
Proof. vm_compute. reflexivity. Qed.
...
Example subscript_body :
  ...
  Some (Sseq [
    Sreturn (Some
      (Eunresolved_binop Rsubscript (Evar "p" (Tparam "T")) (Eint 0%Z Tint)))
  ]).
Proof. vm_compute. reflexivity. Qed.
...
Example pointer_to_member_body :
  ...
  Some (Sseq [
    Sreturn (Some
      (Eunresolved_binop (Rbinop Bdotp)
        (Evar "obj" (Tparam "T"))
        (Evar "member" (Tmember_pointer (Tparam "T") Tint))))
  ]).
Proof. vm_compute. reflexivity. Qed.
