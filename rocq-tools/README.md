...
and performance data when compiling Rocq projects efficiently with dune. As it
...
To set up data gathering, one must tell dune to use our `rocq-perf` wrapper in
the place of plain `rocq`. This is achieved by setting up an `env` stanza like
...
- Per-command performance data captured using Rocq's profiling mechanism, with
...
- A copy of what the wrapped invocation of `rocq` printed on its `stderr`, and
...
The embedding of data into `.glob` files is managed by the `Rocq_tools.Globfs`
...
As the data we collect includes the standard and error output of `rocq`, it is
possible to work around [dune!7460](https://github.com/ocaml/dune/issues/7460)
and list all Rocq warnings for the project, even when there are cache hits.
...
```
$ (cd _build/default && rocq-perf.report .) > full_warnings.log
$ (cd _build/default && rocq-perf.report dir1 dir2) > partial_warnings.log
```
The script also emits additional warnings to indicate whenever the compilation
of a Rocq source file produced non-empty standard output. This can be used for
detecting, e.g., that a `Search` or `About` command was forgotten somewhere.
...
```
$ cat full_warnings.log | rocq-perf.code-quality-report > code-quality.json
```
...
```
$ rocq-perf.extract-all _build/default data
```
...
performance summary for all processed Rocq files. Given two such CSV files one
can generate a comparison table as follows.
```
$ rocq-perf.summary-diff data_ref/perf_summary.csv data/perf_summary.csv
...
Use `rocq-perf.summary-diff --help` to learn more about the available options.
...
```
$ rocq-perf.html-diff-alll data_ref data report
```
