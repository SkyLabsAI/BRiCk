        ...
        (fun replacement_path ->
          ...
          let argv =
            [| "diff"
             ...
             ; replacement_path |]
          in
          let channel = Unix.open_process_args_in "diff" argv in
          ...
          | Unix.WEXITED code ->
              failf "diff failed for %s with exit code %d" display code
          | Unix.WSIGNALED signal | Unix.WSTOPPED signal ->
              failf "diff failed for %s with signal %d" display signal ) )
